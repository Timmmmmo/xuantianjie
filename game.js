...
const ui = {
  ...
  btnRevive: $("btnRevive"),
  btnShare: $("btnShare"),
  btnSignin: $("btnSignin"), signinCycle: $("signinCycle"), signinStatus: $("signinStatus"),
...
function endRun(opts) {
  ...
  releaseJoystick();
  releaseWakeLock();
  if (window.Tutorial && Tutorial.isActive) Tutorial.finish(true);
...
function refreshSigninUI() {
  if (!ui.btnSignin || !window.Signin) return;
  const meta = Meta.load();
  const st = Signin.status(meta);
  if (ui.signinStatus) {
    ui.signinStatus.textContent = st.canClaim
      ? `可领 +${st.reward}`
      : `今日已领 · 明日 ${st.nextDay}`;
  }
  if (ui.signinCycle) {
    ui.signinCycle.innerHTML = st.cycle.map((c) => {
  ...
  if (ui.btnSignin) {
    ui.btnSignin.disabled = !st.canClaim;
    const locked = st.already ? `今日已领 · 明日 ${st.nextDay}` : `领取签到 +${st.reward}`;
    ui.btnSignin.querySelector("span").textContent = st.canClaim ? `领取签到 +${st.reward}` : locked;
  }
}
...
if (ui.btnShare) {
  ui.btnShare.addEventListener("click", async () => {
    ...
    if (window.Analytics) Analytics.track("share_click", { mode: G.mode, wave: G.wave });
